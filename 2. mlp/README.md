...
- `charts/`: PNG and SVG charts generated by `notebooks/featurewise_mlp_quality_scoring_model.ipynb`, including training-data diagnostics, MLP training curve, selected multiplier groups, and per-project candidate curves. The notebook is a standalone replica of the final MLP model-building code path, and its selected multipliers were checked against the final CSV.
