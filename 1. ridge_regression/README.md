...
- `charts/`: PNG and SVG charts generated by `notebooks/featurewise_ridge_quality_scoring_model.ipynb`, including training-data diagnostics, Ridge lambda search, selected multiplier groups, and per-project candidate curves. The notebook is a standalone replica of the final Ridge model-building code path, and its selected multipliers were checked against the final CSV.
